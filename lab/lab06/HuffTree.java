// H343 / Spring 2017 Lab 06 Andrew Mathies awmathie
...
public class HuffTree<Key, E> {
    ...
    private void helper(BinNode<Integer, String> node, String code) {
	if (!node.isLeaf()) {
	    helper(node.getLeft(), code + '0');
	    helper(node.getRight(), code + '1');
	} else
	...
	while (num < inCode.length()) {
	    while (!currentNode.isLeaf()) {
		if (inCode.charAt(num) == '0') {
